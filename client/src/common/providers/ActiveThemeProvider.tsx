...
export function ActiveThemeProvider({ children, initialTheme }: { children: ReactNode; initialTheme?: string }) {
	...
	useEffect(() => {
		...
		if (activeTheme.endsWith('-scaled')) document.body.classList.add('theme-scaled')
	}, [activeTheme])
...
export function useThemeConfig() {
	...
	if (context === undefined) throw new Error('useThemeConfig must be used within an ActiveThemeProvider')
