...
export const FormSendMail = () => {
	...
	async function onSubmit(values: z.infer<typeof formSchema>) {
		...
		try {
			...
			const hasFiles = (attachmentType === 'file' || attachmentType === 'both') && uploadedFiles.length > 0
			const hasUrls = (attachmentType === 'url' || attachmentType === 'both') && values.attachments.length > 0

			// Preparar los datos del correo
			if (hasFiles) {
				// Usamos FormData cuando hay archivos adjuntos
				const formData = new FormData()

				// Agregar datos básicos
				formData.append('from', values.fromMail)
				formData.append('subjectEmail', values.subjectEmail)
				formData.append('message', values.messageBody)

				// Agregar cada correo por separado
				values.sendTo.forEach(email => {
					formData.append('sendTo[]', email) // Cambiado de 'sendTo[]' a 'sendTo'
				})

				// Agregar URLs de adjuntos si hay
				if (hasUrls) {
					const urlAttachments = values.attachments.map(path => ({
				...
				console.log('Enviando FormData con archivos adjuntos y/o URLs')
				console.log('DATA', formData)

				// Enviar con FormData
				const response = await axios.post(`${process.env.NEXT_PUBLIC_URL_API}/email/send`, formData, {
					headers: {
						'x-key-emitto': values.secretKey,
						'Content-Type': 'multipart/form-data', // Asegúrate de incluir este header
					},
				})

				console.log('Respuesta del servidor:', response.data)
			} else {
				// Si solo hay URLs o ningún adjunto, usamos JSON
				const attachments = hasUrls
					? values.attachments.map(path => ({
							filename: path.split('/').pop() || 'file.pdf',
							path: path,
						}))
					: []
				...
				console.log('Enviando JSON con datos:', emailData)

				// Enviar con JSON
				const response = await axios.post(`${process.env.NEXT_PUBLIC_URL_API}/email/send`, emailData, {
					headers: {
						'X-Key-Emitto': values.secretKey,
					},
				})
